...
export const useWork = () => {
    ...
    const getWorks = async (): Promise<AxiosResponse<GetWorkType[]>> => {
        try {
            const response: AxiosResponse<GetWorkType[]> = await axios.get(REACT_APP_BACKEND_URL + "/work/get");
            return response;
        } catch (error) {
    ...
    const getDetailWork = async (id: string): Promise<AxiosResponse<GetWorkType>> => {
        try {
            const response: AxiosResponse<GetWorkType> = await axios.get(REACT_APP_BACKEND_URL + `/work/${id}`);
            return response;
        } catch (error) {
            alert(`取得に失敗しました。${error}`);
            throw error;
